...
`).map(f=>f.trim()).filter(Boolean))}return l})}return s}function Gd(e){return M(e),null}var z,Ma,Jr,Wd,$s,ws,ec=O(()=>{B();W();Q();ce();J();le();ee();z=(e,a)=>w()?e:a,Ma=[{key:"about",icon:"store",blocks:["hero","sections","stats"]},{key:"guide",icon:"info",blocks:["hero","steps","tips"]},{key:"service",icon:"headset",blocks:["hero","items"]},{key:"faq",icon:"help",blocks:["faq"]},{key:"terms",icon:"file",blocks:["hero","intro","notice","sections"]},{key:"privacy",icon:"shield",blocks:["hero","intro","sections"]},{key:"insurance",icon:"package-check",blocks:["hero","body","rules"]},{key:"ticketRules",icon:"ticket",blocks:["hero","rules"]},{key:"bugReport",icon:"bug",blocks:["hero","body","hints"]},{key:"contact",icon:"phone",blocks:["hero"]}],Jr={};Wd=()=>({title:"",titleEn:"",body:"",bodyEn:"",list:[],listEn:[]});$s=()=>r`
  ...
  <button type="button" class="btn btn-ghost btn-icon btn-xs" data-act="adm-pg-down" aria-label="down">${c("chevron-down")}</button>`,ws=()=>r`<button type="button" class="btn btn-ghost btn-icon btn-xs" data-act="adm-pg-del" aria-label="delete">${c("trash")}</button>`;g("adm-pg-save",async(e,a)=>{e.preventDefault();let s=a.dataset.key,n=Ma.find(i=>i.key===s),o=Yd(a,n);await A(a.querySelector("button[type=submit]"),async()=>{try{await h.patch(`/api/admin/pages/${s}`,{value:o}),$(t("misc.saved")),T(!0)}catch(i){y(i)}})});g("adm-pg-add",(e,a)=>{let s=a.dataset.kind,n=a.closest("form"),o=n.querySelector(`[data-tpl="${s}"]`),i=n.querySelector(`[data-blocks="${s}"]`);if(!o||!i)return;let l=document.createElement("div");l.innerHTML=o.innerHTML.trim();let m=l.firstElementChild;if(!m)return;m.querySelectorAll("input, textarea").forEach(v=>{v.value=""});let u=i.children.length+1,f=m.querySelector(".row strong");f&&(f.textContent=`${s==="faq"?w()?"\u0633\u0624\u0627\u0644":"Q":w()?"\u0645\u0648\u0631\u062F":"Item"} ${u}`),i.appendChild(m),m.scrollIntoView({behavior:"smooth",block:"center"})});g("adm-pg-del",(e,a)=>{a.closest("[data-block]")?.remove()});g("adm-pg-up",(e,a)=>{let s=a.closest("[data-block]");s?.previousElementSibling&&s.parentNode.insertBefore(s,s.previousElementSibling)});g("adm-pg-down",(e,a)=>{let s=a.closest("[data-block]");s?.nextElementSibling&&s.parentNode.insertBefore(s.nextElementSibling,s)});g("adm-pg-li-add",(e,a)=>{let s=a.closest(".span-2"),n=s.querySelector("[data-li]"),o=s.querySelector("[data-li-tpl]");if(!n||!o)return;let i=document.createElement("div");i.innerHTML=o.innerHTML.trim(),i.firstElementChild&&n.appendChild(i.firstElementChild)});g("adm-pg-list-toggle",(e,a)=>{let s=a.closest(".span-2")?.querySelector("[data-listbox]");s&&(s.hidden=!s.hidden)});g("adm-pg-lidel",(e,a)=>{a.closest("[data-lir]")?.remove()})});var Yn={};G(Yn,{mount:()=>tm,render:()=>Kd});async function Kd(e){return e.params.section==="imageSearch"?em():Qd()}async function Qd(){try{let a=await h.get(h.url("/api/admin/barcode",{q:dt.q}));Tt=a.items||[],jt=a.labelSizes||[],!jt.find(s=>s.id===dt.size)&&jt.length&&(dt.size=jt[0].id)}catch(a){return F({title:a?.message||t("err.generic")})}let e=Tt.filter(a=>!a.barcode).length;return r`
    ...
    ${R("plus")?r`<span class="f-badge">${c("sparkles")}<span>${t("footer.plus")}</span></span>`:""}`;let a=e.socials||{},s=[["instagram",qe(a.instagram),"camera"],["telegram",qe(a.telegram),"send"],["whatsapp",a.whatsapp?`https://wa.me/${String(a.whatsapp).replace(/\D/g,"")}`:"","chat"],["eitaa",qe(a.eitaa),"globe"]].filter(n=>n[1]);N("#fSocial").innerHTML=s.length?s.map(([n,o,i])=>r`<a href="${o}" target="_blank" rel="noopener noreferrer" aria-label="${n}" title="${n}">${c(i)}</a>`).join(""):r`<span class="muted tiny">${t("footer.contactUs")}</span>`,N("#fContact").innerHTML=r`
