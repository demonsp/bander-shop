...
(async () => {
  ...
  ok('تم پیش‌فرض رنگ #31afd4', boot.json.settings?.theme?.accent === '#31afd4', boot.json.settings?.theme?.accent);
  ...
  // ── ۳) امنیت ──────────────────────────────────────────
  section('امنیت');
  const noCsrf = await reqNoCsrf(anon, 'POST', '/api/auth/login', { identifier: 'admin', password: 'Bander@1404' });
  ...
  ok('ورود مدیر برای کلید کپچا', (await req(capAdmin, 'POST', '/api/auth/login', { identifier: 'admin', password: 'Bander@1404', captchaToken: capPreV.json?.token })).status === 200);
  ...
  const adminLogin = await req(admin, 'POST', '/api/auth/login', { identifier: 'admin', password: 'Bander@1404' });
  ...
  // تنظیمات و تم
  const setTheme = await req(admin, 'PATCH', '/api/admin/settings/theme', { value: { accent: '#31afd4', portTheme: false } });
  ok('تغییر تم (خاموش کردن پوستهٔ بندری)', setTheme.status === 200 && setTheme.json.value.portTheme === false);
  ...
  const addr = await req(mary, 'POST', '/api/me/addresses', { title: 'خانه', receiver: 'مریم احمدی', phone: '09171234567', province: 'بوشهر', city: 'بوشهر', zone: 'city', street: 'خیابان آزمایشی، پلاک ۱', postal: '7512345678', isDefault: true });
