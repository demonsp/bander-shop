...
await login(admin, 'admin', 'Bander@1404');
log.push('✔ ورود مدیر');
...
await addAddress(maryam, {
  ...
  province: 'بوشهر', city: 'بندر گناوه', zone: 'province',
...
step('نظر مریم (۵ ستاره)', await maryam.post('/api/reviews', {
  ...
  body: 'کالا اصل و پلمپ بود. بسته‌بندی خیلی خوب بود و دو روزه به بندر گناوه رسید. حتماً دوباره خرید می‌کنم.',
}));
...
step('پیشنهاد مریم', await maryam.post('/api/feedback', {
  ...
  body: 'خیلی از مشتری‌های بندر کارت به کارت را ترجیح می‌دهند. اگر شمارهٔ کارت و نام دارنده را در صفحهٔ پرداخت بگذارید عالی می‌شود.',
...
await addAddress(reza, {
  ...
  province: 'بوشهر', city: 'بوشهر', zone: 'city',
...
await addAddress(sina, {
  ...
  province: 'بوشهر', city: 'خورموج', zone: 'province',
...
const gOrder = step('ثبت سفارش مهمان (حضوری + پرداخت آنلاین)', await guest.post('/api/checkout', {
  ...
  guestName: 'مهمان بندر گناوه', guestPhone: '09011234567',
}));
if (gOrder?.order?.id) {
...
const cp = step('ساخت کد تخفیف جدید', await admin.post('/api/admin/coupons', {
  ...
  usageLimit: 50, perUser: 1, active: true, note: 'تخفیف ویژهٔ مشتریان بندر گناوه',
}));
...
step('ارسال اعلان سراسری', await admin.post('/api/admin/notifications', {
  ...
  body: 'این هفته ارسال سفارش‌های بالای ۲٬۵۰۰٬۰۰۰ تومان به کل استان بوشهر رایگان است. کد تخفیف GENAVEH15 هم فعال شد.',
